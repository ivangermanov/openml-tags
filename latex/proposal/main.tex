...
\title{Topic Modelling of OpenML Dataset Descriptions}
...
% - Speak about originality and innovative character - nobody has applied topic modelling on dataset descriptions.
...
\begin{itemize}
  \item (\textbf{A}) Literature analysis - This involves thorough reading of the relevant literature on topic modelling. \textbf{Milestone:} Better understanding of current state-of-the-art topic models, and gaining deeper insight into methods for modeling the OpenML dataset descriptions.
...
\citet{abdelrazek_topic_2022} discuss several evaluation criteria - namely, quality, interpretability, stability, diversity, efficiency and flexibility (\cref{fig:evaluation_criteria}).

Here, we will focus on quality, interpretability and diversity, as they are most appropriate for our use case.
...
A topic is a discrete distribution over words. This set of words is evaluated for being human interpretable. For this to occur, the generated words should be associated with a single semantic meaning.
...
This metric describes how semantically diverse the obtained topics are. One way to define diversity is introduced in \cite{dieng_topic_2020}. It defines topic diversity as the percentage of unique words in the top 25 words of all topics. A topic model should generate diverse topics and score high on this metric. A low score indicates redundant topics, meaning the model could not sufficiently disentangle the corpus’s themes. Note that selecting a number of topics in the model affects the topic diversity. A too large number could result in similar topics with overlapping top words. A too small number could result in broad topics with poor interpretability.
...
It is important to note that the evaluation metrics above concern topic modelling as an unsupervised task. If after experimentation we find that our proposed BERTopic model does not perform sufficiently well, we may turn the problem into a (semi-)supervised problem. In this case, different metrics will apply, so we will cover them briefly.

These would involve coverage - coverage refers to how well the concepts in the document collection are represented. Requires ground truth.

purity - Purity is the accuracy of the model if documents are always assigned the dominant topic. Requires ground truth.

\citet{terragni_octis_2021}
...
\subsubsection{Topic modelling proof of concept using BERTopic}
