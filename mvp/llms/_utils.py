...
def replace_ids(prompt: str, docs: List[str]) -> str:
    # get first line in each doc where id is "- ID: <id>", e.g. "- ID: 1234"
    ids = [doc.split("\n")[0].split(": ")[1] for doc in docs]
    return prompt.replace("[IDS]", ", ".join(ids))
...
def create_prompt(prompt: str, docs: List[str], topic: Any, topics: dict) -> str:
    ...
    if prompt == DEFAULT_CHAT_PROMPT or prompt == DEFAULT_PROMPT:
        ...
        if "[DOCUMENTS]" in prompt:
            prompt = replace_documents(prompt, docs)
        if "[IDS]" in prompt:
            prompt = replace_ids(prompt, docs)
