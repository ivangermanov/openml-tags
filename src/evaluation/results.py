    ...
    def _load_results(self):
        ...
        if "basic" in folders:
            for folder in os.listdir(self.main_folder + "/basic"):
                if os.path.isdir(f"{self.main_folder}/basic/{folder}"):
    ...
    def _load_all_results(self, dtm=False):
        ...
        if dtm:
            ...
            #     print(dataframe.to_string())
                # print(dataframe.groupby("Model").mean())
        keys = list(data.keys())
        if len(keys) > 0:
            sort_key = keys[0]
        else:
            raise ValueError("No data available to sort and align models.")
        order = data[sort_key].sort_values("npmi")["Model"].tolist()
    ...
    def plot_results(
        ...
        for model in results.Model.unique():
            ...
            if confidence_interval:
                ...
                y_mean = selection.groupby('nr_topics')[y].mean()
                x_vals = y_mean.index

                # Compute upper and lower bounds using chosen uncertainty measure: here
                # it is a fraction of the standard deviation of measurements at each
                # time point based on the unbiased sample variance
                y_std = selection.groupby("nr_topics")[y].std()
                error = 0.5 * y_std
                lower = y_mean - error
                upper = y_mean + error
...
def highlight_max(data):
    """
    Highlight the maximum, second to maximum, and third to maximum in a Series or DataFrame.
    """
