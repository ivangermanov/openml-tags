import os
from typing import List, Tuple, Union, Any, Hashable
...
class DataLoader:
    ...
    **Custom Data**
    ...
    def load_docs(self, save: bool = False, docs: List[str] = None) -> Tuple[List[str], Union[List[str], None]]:
        ...
        if self.dataset == "openml":
            self.docs, self.timestamps, self.dropped_indices = self._openml()
        ...
        return self.docs, self.timestamps, self.dropped_indices
    ...
    def preprocess_octis(self, preprocessor: Preprocessing = None, documents_path: str = None,
        ...
        if preprocessor is None:
            preprocessor = Preprocessing(lowercase=False, remove_punctuation=False, # punctuation=string.punctuation,
                                         punctuation='', remove_numbers=False, lemmatize=False, language="english",
                                         split=False, verbose=True, save_original_indexes=True,
                                         remove_stopwords_spacy=False, )
    ...
    def _openml(self) -> tuple[list[Any], None, list[Any]]:
        """Prepare the OpenML dataset and print dropped indices"""
        load_dotenv()
        project_root = os.getenv('PROJECT_ROOT')
        data_path = os.path.join(project_root, 'notebooks', 'data', 'prompt_descriptions.csv')
        openml = pd.read_csv(data_path)

        # Track dropped indices
        dropped_indices = []

        docs = []
        for index, doc in openml['prompt_description'].items():
            if pd.isna(doc) or len(doc) <= 2:
                dropped_indices.append(index)
            else:
                docs.append(doc.replace("\n", " "))

        # Print the indices of dropped rows
        print("Dropped indices:", dropped_indices)

        return docs, None, dropped_indices
